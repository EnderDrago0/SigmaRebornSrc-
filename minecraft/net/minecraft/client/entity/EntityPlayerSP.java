...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void func_175161_p()
    {
        ...
        if (this.func_175160_A())
        {
            ...
            double var9 = (double)((event2.getYaw()) - this.field_175164_bL);// double var9 = (double)(this.rotationYaw - this.field_175164_bL);
            double var11 = (double)((event2.getPitch()) - this.field_175165_bM);// double var11 = (double)(this.rotationPitch - this.field_175165_bM);
            boolean var13 = var3 * var3 + var5 * var5 + var7 * var7 > 9.0E-4D || this.field_175168_bP >= 20;
            boolean var14 = var9 != 0.0D || var11 != 0.0D;
//Jello.addChatMessage(String.valueOf(var11));
            if (this.ridingEntity == null)
            {
                if (var13 && var14)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, this.getEntityBoundingBox().minY, this.posZ, event2.getYaw(), event2.getPitch(), event2.isOnGround()));
                }
                else if (var13)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.getEntityBoundingBox().minY, this.posZ, event2.isOnGround()));
                }
                else if (var14)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(event2.getYaw(), event2.getPitch(), event2.isOnGround()));
                }
                else
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer(event2.isOnGround()));
                }
            }
            else
            {
            ...
            if (var13)
            {
                ...
                this.field_175166_bJ = this.posY;// zelly has this.lastReportedPosY = event.getY();
            ...
            if (var14)
            {
                this.field_175164_bL = event2.getYaw();
                this.field_175165_bM = event2.getPitch();
            }
        }
        EventMotion s = new EventMotion(this.onGround, this.rotationYaw, this.rotationPitch, EventType2.POST);
    	EventManager.call(s);
