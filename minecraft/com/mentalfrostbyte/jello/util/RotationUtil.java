...
public class RotationUtil {
    ...
    public static float[] faceEntityAc(Entity target,Entity current ,float p_70625_2_, float p_70625_3_) {
        double var6;
        double var4 = target.posX - current.posX;
        double var8 = target.posZ - current.posZ;
        if (target instanceof EntityLivingBase) {
            EntityLivingBase var14 = (EntityLivingBase)target;
            var6 = var14.posY + (double)var14.getEyeHeight() - (Minecraft.getMinecraft().thePlayer.posY + (double)current.getEyeHeight());
        } else {
            var6 = (target.getEntityBoundingBox().minY + target.getEntityBoundingBox().maxY) / 2.0 - (current.posY + (double)current.getEyeHeight());
        }
        double var141 = MathHelper.sqrt_double(var4 * var4 + var8 * var8);
        float yaw = (float)(Math.atan2(var8, var4) * 180.0 / 3.141592653589793) - 90.0f;
        float pitch = (float)(- Math.atan2(var6, var141) * 180.0 / 3.141592653589793);
    ...
    private static float getAngleDifferencel(final float a, final float b) {
    ...
    public static boolean rayTrace(final float yaw, final float pitch, final double reach, final Entity target) {
        final Vec3 vec3 = Minecraft.getMinecraft().thePlayer.func_174824_e(Minecraft.getMinecraft().timer.renderPartialTicks);
        final Vec3 vec31 = Minecraft.getMinecraft().thePlayer.getVectorForRotationFloat(MathHelper.clamp_float(pitch, -90.F, 90.F), yaw % 360);
    ...
    public static MovingObjectPosition getMovingObjectPositionFromVector(float yaw, float pitch) {
        Vec3 eyesPos = new Vec3(Minecraft.getMinecraft().thePlayer.posX, Minecraft.getMinecraft().thePlayer.posY + Minecraft.getMinecraft().thePlayer.getEyeHeight(), Minecraft.getMinecraft().thePlayer.posZ);
        Vec3 rotationVector = RotationUtil.getVectorForRotation(yaw, pitch);
        Vec3 vector = eyesPos.addVector(rotationVector.xCoord * 4, rotationVector.yCoord * 4, rotationVector.zCoord * 4);

        return Minecraft.getMinecraft().theWorld.rayTraceBlocks(eyesPos, vector, false, false, true);
    }
