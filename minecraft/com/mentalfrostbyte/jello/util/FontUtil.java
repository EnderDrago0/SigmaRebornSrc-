...
public class FontUtil {
	...
	private static Font san(int size) {
		Font font = null;
		try {
			InputStream is = Minecraft.getMinecraft().getResourceManager()
					.getResource(new ResourceLocation("Jello/sfui.ttf")).getInputStream();
			font = Font.createFont(0, is);
			font = font.deriveFont(0, size);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public static JelloFontRenderer san = JelloFontRenderer.createFontRenderer(san(24));
