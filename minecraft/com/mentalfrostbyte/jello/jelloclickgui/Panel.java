...
public class Panel {
	...
	public void mouseClicked(int mouseX, int mouseY, int mouseButton, JelloGui parent){
		...
		int f = 0;
		 ...
		 for(File fi : Jello.scripts) {
			 if(category.index == 7) {
				 boolean isHovering = isHoveringCoords(x, y + 25 + f*15 - scroll, 100, 15, mouseX, mouseY) && this.isMouseHoveringRect2(x - 19/2f + 9.5f, y - 19/2f + 9.5f + 25, x - 19/2f - 9.5f + 238/2f, y - 19/2f - 9.5f + 338/2f, Jello.jgui.mouseX, Jello.jgui.mouseY);
				 if(isHovering){
						ScriptUtils.toggle(fi);
			     }
				 f++;
			 }
		 }
	...
	public void drawPanel(int mouseX, int mouseY, float opacity, boolean hideShadow, float leftIntro, float rightIntro){
		...
		if(!hideShadow){
		Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation(ClickGUI.theme.is("Light") ? "Jello/JelloPanel.png" : "Jello/JelloPanel1.png"));
	   ...
	   // 	float hoverTrans = smoothTrans(m.hoverPercent, m.lastHoverPercent);
	    	if(category.index == 7) {
	    		...
	    		FontUtil.jelloFontAddAlt3.drawNoBSCenteredString(f.getName(), x + 100/2f, y + 58/2f + fcount*15 - scroll, new Color(0.90f, 0.90f, 0.90f, opacity).getRGB());
		    	if(ClickGUI.theme.is("Light"))
		    		Gui.drawFloatRect(hideShadow ? leftIntro : x, y + 25 + fcount*15 - scroll, hideShadow ? rightIntro :  x + 100/* + (hideShadow ? 1.5f*(1-intro) : 0)*/, y + 25 + 15 + fcount*15 - scroll, ScriptUtils.get(f) ? new Color(42, 165, 255, (int)(opacity*(200 + (55*1)))).getRGB() : new Color(50,20, 250, (int)(opacity*255)).getRGB());
		    	else if(ClickGUI.theme.is("Dark"))
		    		Gui.drawFloatRect(hideShadow ? leftIntro : x, y + 25 + fcount*15 - scroll, hideShadow ? rightIntro :  x + 100/* + (hideShadow ? 1.5f*(1-intro) : 0)*/, y + 25 + 15 + fcount*15 - scroll, ScriptUtils.get(f) ? new Color(42, 165, 255, (int)(opacity*(200 + (55*1)))).getRGB() : new Color(30, 30, 30, (int)(opacity*255)).getRGB());
	    ...
	    for(Module m : Jello.getModulesInCategory(category)){
	    	...
	    	//small rects unfocused//
	    	
	    	
	    	
	    	
	    	Color h = Color.getHSBColor(270, 0, 0.8f + (0.16f*hoverTrans));//Color h = Color.getHSBColor(270, 0, 0.8f + (0.16f*hoverTrans));
	    	if(ClickGUI.theme.is("Light"))
	    		...
	    		Gui.drawFloatRect(hideShadow ? leftIntro : x, y + 25 + count*15 - scroll, hideShadow ? rightIntro :  x + 100/* + (hideShadow ? 1.5f*(1-intro) : 0)*/, y + 25 + 15 + count*15 - scroll, m.isToggled() ? new Color(42, 165, 255, (int)(opacity*(200 + (55*hoverTrans)))).getRGB() : new Color(30, 30, 30, (int)(opacity*255)).getRGB());
	    	...
	    	if(ClickGUI.theme.is("Light"))
	    		...
	    		FontUtil.jelloFontAddAlt3.drawNoBSCenteredString(m.getName(), x + 100/2f, y + 58/2f + count*15 - scroll, m.isToggled() ?new Color(0.90f, 0.90f, 0.90f, opacity).getRGB() : new Color(1f, 1f, 1f, opacity).getRGB());
	    	count++;
	    }
