...
import com.mentalfrostbyte.jello.modules.ClickGUI;
...
public class JelloGui extends GuiScreen {
	...
	public float lastSettingsTransi;
	public float settingsTransi;
	...
	private ParticleEngine pi = new ParticleEngine();
	private ParticleEngine pu = new ParticleEngine();
	public Music music;
	private int current;
	public JelloGui(){
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    	...
    	for(Value v : selectedModule.getValues()){
    		...
    		if(v instanceof ModeValue) {
    			ModeValue m = (ModeValue)v;
    			if(this.isHoveringCoords(popupLeft + 100, popupTop + 25 + 10 + (34/2f)*y,  60, 12, mouseX, mouseY)){
    				m.cycle();
    			}
    		}
    		y++;
    ...
    public void onTick(EventTick e){
    	...
    	lastSettingsTransi = settingsTransi;
    	lastPercent = percent;
    	...
    	if(selectedModule != null){
    		...
    		if(this.settingsTransi < 1 && !exit){
    			settingsTransi += ((1-settingsTransi)/(1.8f))+0.01;
    			//settingsTrans += 0.1;
    		}
    		if(exit && settingsTransi > 0){
    			settingsTransi += ((0-settingsTransi)/(1.8f))-0.01;
    		}
...
public void drawShadow(float x, float y, float x2, float y2, float width, float height){
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    	this.mouseX = mouseX;
    	this.mouseY = mouseY;
    	if(ClickGUI.particles.isEnabled()) {
	        pe.render(Display.getWidth() / 2, Display.getHeight() / 3);
	        pi.render(Display.getWidth() / 2, Display.getHeight() / 3);
	        pu.render(Display.getWidth() / 2, Display.getHeight() / 3);
    	}
    	if (Mouse.hasWheel()) {
    	...
    	if(selectedCategory != null){
    		...
    		float popupTop = selectedCategory.y;
    		float intro = smoothTrans(settingsTrans, lastSettingsTrans);
    		float i = smoothTransi(settingsTransi, lastSettingsTransi);
    		float leftOffset = 60*intro - 60;
    		float popupLeft = Math.max(0, selectedCategory.x - (30*intro)) - Math.max(0, (selectedCategory.x - (30*intro)) - (sr.getScaledWidth() - (100 + (60)*intro)));
    		
    		//change background color when settings of a module is on
    		Gui.drawFloatRect(0, 0, sr.getScaledWidth(), sr.getScaledHeight(), new Color(0, 0, 0, (int)(intro*140)).getRGB());
    		//original
 		    ...
 		    */
 	    	...
 	    	Stencil.write(false);
 	    	Gui.drawFloatRect(popupLeft, popupTop, popupLeft + 100 + (60)*intro/* + 1.5f*(1-intro)*/, popupTop + 150 + (14.5f)*intro/* + 1f*(1-intro)*/, ClickGUI.theme.is("Light") ? -1 : 0xff1e1e1e);
 	    	Stencil.erase(false);
 	    	drawShadow(popupLeft, popupTop, 0, 0, 100 + (60)*intro, 150 + (14.5f)*intro);
 	    	Stencil.dispose();
 		    
    		Gui.drawFloatRect(popupLeft, popupTop + 25, popupLeft + 100 + (60)*intro/*+ 1.5f*(1-intro)*/, popupTop + 150 + (14.5f)*intro/* + 1f*(1-intro)*/, ClickGUI.theme.is("Light") ? -1 : 0xff1e1e1e);
    		GlStateManager.disableAlpha();
 	    	GlStateManager.enableBlend();
 	    	GL11.glEnable(3042);
 	    	GL11.glColor4f(1, 1, 1, 1);
    		...
    		if(ClickGUI.theme.is("Light"))
    			Gui.drawFloatRect(popupLeft, popupTop, popupLeft + 100 + (60)*intro/*+ 1.5f*(1-intro)*/, popupTop + 25, 0xff99989a);
    		else if(ClickGUI.theme.is("Dark"))
    			Gui.drawFloatRect(popupLeft, popupTop, popupLeft + 100 + (60)*intro/*+ 1.5f*(1-intro)*/, popupTop + 25, 0xff1e1e1e);
    	...
    	for(Value v : selectedModule.getValues()){
    		...
    		if(v instanceof BooleanValue){
    			...
    			FontUtil.jelloFontGui.drawNoBSString(b.getName(), popupLeft + 7, popupTop + 25 + 21/2f + (34/2f)*y, ClickGUI.theme.is("Light") ? new Color(0, 0, 0, intro).getRGB() : new Color(1, 1, 1, intro).getRGB());
    		...
    		if(v instanceof ModeValue) {
    			...
    			FontUtil.jelloFontGui.drawNoBSString(m.getName()+ "            " + m.getMode(), popupLeft + 7, popupTop + 25 + 21/2f + (34/2f)*y, ClickGUI.theme.is("Light") ? new Color(0, 0, 0, intro).getRGB() : new Color(1, 1, 1, intro).getRGB());
    			//FontUtil.jelloFontGui.drawNoBSString(m.getMode(), popupLeft + 7, popupTop + 25 + 21/2f + (34/2f)*y, ClickGUI.theme.is("Light") ? new Color(0, 0, 0, intro).getRGB() : new Color(1, 1, 1, intro).getRGB());
    		}
    		if(v instanceof NumberValue){
    			NumberValue n = (NumberValue)v;
    			float popupTop2 = popupTop - 0.5f + 5.5f;
    		    ...
    		    }//FontUtil.jelloFontGui.drawString(String.valueOf(n.getValue()), sliderLeft - 20, popupTop2 + 25 + 10 + (34/2f)*y - 4.5f, (int)-1);
    		    if((hoverEffect > 1) || this.isHoveringCoords((float) (sliderLeft + (52 - 11)*(actualPercent) - 4.5f- 1.5f) + leftOffset, popupTop2 + 25 + 10 + (34/2f)*y - 4.5f, 23/2f, 25/2f, mouseX, mouseY)){
    		    	//GlStateManager.color(0.9f, 0.9f, 0.9f, 1f);
        			GlStateManager.translate((float) (sliderLeft + leftOffset + (52 - 11)*(actualPercent) - 4.5f- 1.5f) + 23/4f, popupTop2 + 25 + 10 + (34/2f)*y - 4.5f + 25/4f, 0);
        			...
        			GlStateManager.translate(-((float) (sliderLeft  + leftOffset+ (52 - 11)*(actualPercent) - 4.5f- 1.5f) + 23/4f), -(popupTop2 + 25 + 10 + (34/2f)*y - 4.5f + 25/4f), 0);
    		    	FontUtil.jelloFontGui.drawString(String.valueOf(n.getValue()), sliderLeft - 20, popupTop2 + 25 + 10 + (34/2f)*y - 4.5f, ClickGUI.theme.is("Light") ? 1 : -1);
    		    }
    		    
    		    
    			FontUtil.jelloFontGui.drawNoBSString(n.getName(), popupLeft + 7, popupTop + 25 + 21/2f + (34/2f)*y, ClickGUI.theme.is("Light") ? new Color(0, 0, 0, intro).getRGB() : new Color(1, 1, 1, intro).getRGB());
    		}
    		y++;
    ...
    public float smoothTransi(double current, double last){
		return (float) (current * Minecraft.getMinecraft().timer.renderPartialTicks + (last * (1.0f - Minecraft.getMinecraft().timer.renderPartialTicks)));
	}
    ...
    public void initGui() {
    	...
    	if (!(mc.gameSettings.ofFastRender) && OpenGlHelper.shadersSupported && this.mc.getRenderViewEntity() instanceof EntityPlayer && ClickGUI.blur.isEnabled()) {
			if (this.mc.entityRenderer.theShaderGroup != null)
				this.mc.entityRenderer.theShaderGroup.deleteShaderGroup();
			this.mc.entityRenderer.applyShader(new ResourceLocation("shaders/post/blur.json"));
		}
    	percent = 1.23f;
		lastPercent = 1.23f;
		percent2 = 0.98f;
